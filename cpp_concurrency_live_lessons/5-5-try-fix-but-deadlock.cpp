...
class Bank
{
    ...
    void transfer(int from, int to, int sum) {
         // while this looks like it resolves the problem it actually
         // causes a deadlock....sometimes 
         // the order in which these locks are picked is not guaranteed!!!!!
        std::lock_guard<recursive_mutex> lck1(_accts[from].getMutex());
        std::lock_guard<recursive_mutex> lck2(_accts[to].getMutex());
